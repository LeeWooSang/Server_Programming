...
bool Initialize()
{
	...
	// Socket( )
	// 대기 소켓 생성
	//SOCKET listen_socket = WSASocket(AF_INET, SOCK_STREAM, IPPROTO_TCP, nullptr, , 0);
	...
	while (true)
	{
		...
		while (true)
		{
			retval = send(client_socket, (char*)&sc_packet, sizeof(sc_packet), 0);
			if (retval == SOCKET_ERROR)
			{
				err_display("send( )");
				break;
...
//	cout << sc_packet.m_X << ", " << sc_packet.m_Y << endl;
}